...
class DocumentHandler:
    ...
    def __init__(self, session_id=None, data_dir=None):
        try:
            ...
            raise DocumentPortalException(f"Error initializing DocumentHandler: {e}") from e
    ...
    def save_pdf(self, uploaded_file):
        try:
            ...
            raise DocumentPortalException(f"Error saving PDF: {e}") from e
    ...
    def read_pdf(self, pdf_path: str) -> str:
        try:
            ...
            text = "\n".join(text_chunks)
            self.log.info(f"PDF read successfully. pdf_path={pdf_path}, session_id={self.session_id}, pages={len(text_chunks)}")
            return text
        except Exception as e:
            self.log.error(f"Error reading PDF: {e}")
            raise DocumentPortalException(f"Error reading PDF: {e}") from e    

if __name__ == "__main__":
    from pathlib import Path
    pdf_path = r"D:\\WORK\\End_to_End_Projects\\Rag_Project\\data\\sample.pdf"
